...
#define SM3BlockSize 64
#define SM3HashSize 32
...
static WORD FF(WORD x, WORD y, WORD z, int j) { return j<=15 ? x^y^z : (x&y)|(x&z)|(y&z); }
static WORD GG(WORD x, WORD y, WORD z, int j) { return j<=15 ? x^y^z : (x&y)|(~x&z); }
static WORD P0(WORD x) { return x ^ ROTL(x,9) ^ ROTL(x,17); }
static WORD P1(WORD x) { return x ^ ROTL(x,15) ^ ROTL(x,23); }

// 消息扩展
static void extendMessage(const BYTE* block, WORD* W, WORD* W1) {
    for(int i=0;i<16;i++)
...
// 压缩函数（无任何优化）
static void compress(WORD* V, const BYTE* block) {
    WORD W[68], W1[64];
    extendMessage(block,W,W1);

    WORD A=V[0],B=V[1],C=V[2],D=V[3];
    WORD E=V[4],F=V[5],G=V[6],H=V[7];

    for(int j=0;j<64;j++){
        WORD SS1 = ROTL((ROTL(A,12)+E+ROTL(T[j],j)) & 0xFFFFFFFF,7);
        WORD SS2 = SS1 ^ ROTL(A,12);
        WORD TT1 = (FF(A,B,C,j)+D+SS2+W1[j]) & 0xFFFFFFFF;
        WORD TT2 = (GG(E,F,G,j)+H+SS1+W[j]) & 0xFFFFFFFF;
...
void sm3Hash(const BYTE* msg, size_t msgLen, BYTE* digest) {
    ...
    for(size_t i=0;i<newLen;i+=64) compress(V,padded+i);
...
void throughputTest() {
    ...
    BYTE digest[SM3HashSize];

    clock_t start = clock();
    sm3Hash(data,testSize,digest);
    clock_t end = clock();
    double seconds = (double)(end-start)/CLOCKS_PER_SEC;
    printf("Processed %zu bytes in %.3f seconds, throughput %.2f MB/s\n",
...
int main() {
    ...
    printf("SM3(\"%s\") = ",msg);
    for(int i=0;i<SM3HashSize;i++) printf("%02x",digest[i]);
    printf("\n");
