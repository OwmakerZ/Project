#include "sm4.h"
#include <stdint.h>
#include <string.h>

#define SM4_ROUNDS 32

// 系统参数 FK
static const uint32_t Sm4FK[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};

// 固定常量 CK
static const uint32_t Sm4CK[SM4_ROUNDS] = {
    0x00070e15,0x1c232a31,0x383f464d,0x545b6269,
    0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
    0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,
    0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
    0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,
    0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
    0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,
    0x10171e25,0x2c333a41,0x484f565d,0x646b7279
};

// S-box
static const uint8_t Sm4SBoxT[256] = {
    
};

// 预计算 T-table
static uint32_t SM4_T[256];

void Sm4InitTTable(void) {
    for (int i = 0; i < 256; i++) {
        uint8_t b = Sm4SBoxT[i];
        uint32_t t = ((uint32_t)b << 24) | ((uint32_t)b << 16) | ((uint32_t)b << 8) | b;
        uint32_t t1 = t ^ ((t << 2) | (t >> 30)) ^ ((t << 10) | (t >> 22)) ^
                      ((t << 18) | (t >> 14)) ^ ((t << 24) | (t >> 8));
        SM4_T[i] = t1;
    }
}

// 轮密钥生成
void Sm4TKeySchedule(const uint8_t key[16], uint32_t rk[SM4_ROUNDS]) {
    uint32_t K[36];
    for (int i = 0; i < 4; i++) {
        K[i] = ((uint32_t)key[4*i] << 24) |
               ((uint32_t)key[4*i+1] << 16) |
               ((uint32_t)key[4*i+2] << 8) |
               ((uint32_t)key[4*i+3]);
        K[i] ^= Sm4FK[i];
    }
    for (int i = 0; i < SM4_ROUNDS; i++) {
        uint32_t tmp = K[i+1] ^ K[i+2] ^ K[i+3] ^ Sm4CK[i];
        uint32_t t = SM4_T[(tmp >> 24) & 0xFF] ^
                     SM4_T[(tmp >> 16) & 0xFF] ^
                     SM4_T[(tmp >> 8) & 0xFF] ^
                     SM4_T[tmp & 0xFF];
        K[i+4] = K[i] ^ t;
        rk[i] = K[i+4];
    }
}

// T-Table 加密
void Sm4EncryptBlockTTable(const uint8_t in[16], uint8_t out[16], const uint32_t rk[SM4_ROUNDS]) {
    uint32_t X[36];
    for (int i = 0; i < 4; i++) {
        X[i] = ((uint32_t)in[4*i] << 24) |
               ((uint32_t)in[4*i+1] << 16) |
               ((uint32_t)in[4*i+2] << 8) |
               ((uint32_t)in[4*i+3]);
    }
    for (int i = 0; i < SM4_ROUNDS; i++) {
        uint32_t tmp = X[i+1] ^ X[i+2] ^ X[i+3] ^ rk[i];
        uint32_t t = SM4_T[(tmp >> 24) & 0xFF] ^
                     SM4_T[(tmp >> 16) & 0xFF] ^
                     SM4_T[(tmp >> 8) & 0xFF] ^
                     SM4_T[tmp & 0xFF];
        X[i+4] = X[i] ^ t;
    }
    for (int i = 0; i < 4; i++) {
        uint32_t v = X[35-i];
        out[4*i] = (v >> 24) & 0xFF;
        out[4*i+1] = (v >> 16) & 0xFF;
        out[4*i+2] = (v >> 8) & 0xFF;
        out[4*i+3] = v & 0xFF;
    }
}

// T-Table 解密
void Sm4DecryptBlockTTable(const uint8_t in[16], uint8_t out[16], const uint32_t rk[SM4_ROUNDS]) {
    uint32_t rk_rev[SM4_ROUNDS];
    for (int i = 0; i < SM4_ROUNDS; i++) {
        rk_rev[i] = rk[SM4_ROUNDS-1-i];
    }
    Sm4EncryptBlockTTable(in, out, rk_rev);
}
